# This is a simple hardcoded query to do a single iteration of tool-use with the Gonky droid.
# This is the target query structure that we're aiming to build dynamically in the Python script.
# The initial "messages" array here contains context to help shape the droid's behavior. Note that even though it appears to have responses from the assistant, this is just a simulation of how the droid would respond based on its programming and the tools available to it. This simulation is provided as context for the droid's actions and responses.
# After the LLM's responses are received, they should be added to the message history in the Python script to maintain the conversation state.
# When the LLM responds with tool calls, the Python script should execute those tool calls and update the farm state accordingly.
# The messages array is then updated with the tool call responses, and the next query is sent to the LLM with all of the updated messages / history to get the next response from the LLM.
